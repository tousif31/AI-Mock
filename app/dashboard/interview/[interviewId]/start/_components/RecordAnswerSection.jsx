...
const RecordAnswerSection = ({
  ...
  // Helper function for timeout and retry
  async function callGeminiWithRetry(prompt, maxRetries = 3, timeout = 15000) {
    let lastError;
      ...
      try {
        ...
        const result = await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(() => reject(new Error("AI timed out")), timeout)
          ),
        ]);
        return result;
      } catch (error) {
  ...
  const updateUserAnswer = async () => {
    try {
      ...
      // Use the retry+timeout helper
      const result = await callGeminiWithRetry(feedbackPrompt);
      ...
      try {
        ...
        console.error("Invalid JSON from AI:", MockJsonResp);
        toast("AI feedback could not be parsed. Please try again.");
        setLoading(false);
        return;
      }
      ...
      if (error.message === "AI timed out") {
        toast("The AI is taking too long. Please try again later.");
      } else if (error.message && error.message.includes("503")) {
        toast("The AI model is overloaded. Please try again in a few moments.");
      } else {
